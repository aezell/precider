defmodule PreciderWeb.UserAuthTest do
  use PreciderWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Precider.Accounts
  alias Precider.Accounts.Scope
  alias PreciderWeb.UserAuth

  import Precider.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PreciderWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PreciderWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PreciderWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
