defmodule PreciderWeb.ProductHTML do
  ...
  def product_form(assigns) do
    ...
    ingredient_dosages =
      Map.new(assigns.ingredient_dosages, fn {k, v} ->
    ...
    <.form :let={f} for={@changeset} action={@action} class="space-y-8">
      <div class="space-y-4">
        ...
        <fieldset class="space-y-4">
          ...
          <div
            id="ingredient-options-data"
            data-ingredients={Jason.encode!(@ingredients |> Enum.map(&%{id: &1.id, name: &1.name}))}
            style="display:none;"
          >
          </div>
          <div
            id="ingredient-errors-data"
            data-ingredient-errors={Jason.encode!(@ingredient_errors)}
            style="display:none;"
          >
          </div>
          <div
            id="selected-ingredients-data"
          ...
          <div class="flex justify-between items-center mt-4">
            <button type="button" id="add-ingredient-row" class="btn btn-outline btn-sm ml-4">
      ...
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-base-100 text-base-content rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-base-100 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <.form
              :let={f}
              for={@ingredient_changeset}
              action={~p"/api/products/new/create_ingredient"}
              class="space-y-4"
              id="ingredient-form"
            >
              ...
              <div class="mt-6 flex justify-end space-x-3">
                <button type="button" phx-click="close_ingredient_modal" class="btn btn-ghost">
                  Cancel
                </button>
