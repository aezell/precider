defmodule PreciderWeb.ProductFinderLive.Index do
  ...
  def handle_event("update_ingredient_filter", %{"ingredient_id" => id, "mode" => mode}, socket) do
    ...
    {:noreply,
     socket |> assign(:active_ingredient_filters, updated_filters) |> maybe_update_products()}
  end
  ...
  def handle_event("remove_ingredient_filter", %{"ingredient_id" => id}, socket) do
    ...
    updated_filters =
      Enum.reject(socket.assigns.active_ingredient_filters, &(&1.ingredient_id == ingredient_id))
  ...
  def handle_event(
        ...
        %{"ingredient_id" => _ingredient_id, "selected" => _selected},
        socket
      ) do
  ...
  defp filter_products(products, ingredient_filters) do
    ...
    |> Enum.filter(fn product ->
      ...
      |> Enum.all?(fn filter ->
        ...
        has_ingredient =
          Enum.any?(product.product_ingredients, &(&1.ingredient_id == ingredient_id))
  ...
  defp get_displayed_ingredients(products, active_ingredient_filters, selected_columns) do
    ...
    if active_ingredient_filters == [] do
      ...
      filtered =
        Enum.sort_by(filtered, fn ing -> Enum.find_index(filtered_ids, &(&1 == ing.id)) end)
